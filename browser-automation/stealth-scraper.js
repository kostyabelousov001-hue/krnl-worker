...
function cleanText(text) {
    if (!text) return 'N/A';
    // Удаляем иконки-глифы Google Maps (символы из Private Use Area Unicode: \uE000-\uF8FF)
    const noIcons = text.replace(/[\uE000-\uF8FF]/g, '');
    return noIcons.replace(/\s+/g, ' ').trim();
}

// Extract emails from content string
...
// Block all unnecessary resource types for speed
...
// Extract social links from page DOM
...
// Deep contact finder: Homepage + Contact/About Us fallback
        ...
        if (data.emails.length === 0) {
            ...
            if (contactUrl && contactUrl.startsWith('http')) {
                ...
                if (contactEmails.length > 0) {
                    data.emails.push(...contactEmails);
                }
            }
        }
...
// Extract place details from Google Maps place URL
// Принимает либо строку URL, либо объект { url, rating, reviews } (из Phase 1)
async function extractPlaceDetails(browser, input) {
    ...
    try {
        ...
        if (name && name.length > 1) {
            ...
            // Если рейтинг или отзывы отсутствуют / равны 0, пробуем вытащить со страницы деталей
...
// Parallel batch: extract details from multiple place URLs at once
...
// Parallel batch: find contact details for multiple websites at once
